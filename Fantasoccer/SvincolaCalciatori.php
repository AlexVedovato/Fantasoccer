...
if(!in_array($_GET["id_fantasquadra"],$idFantasquadre)){
  header("Location: ./SvincolaCalciatori.php?error=5");die;
}
?>
<html>
...
<body>
  ...
  <div class="dropdown  my-menu">
    <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
      ...
      $fantasquadraCalciatori = Fantacalcio::getCalciatoriFantasquadra($_GET["id_fantasquadra"]);
      ?>
    </button>
    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
      <?php
      foreach ($fantasquadre as $fantasquadra) {
        echo ' <a class="dropdown-item" href="SvincolaCalciatori.php?id_fantasquadra=' . $fantasquadra["id_fantasquadra"] . '">' . $fantasquadra["nome"] . '</a>';
      }
      ?>
    </div>
  </div>
      ...
      <div class="row">
        ...
        <table id="list" class="display compact" style="width:100%">
          ...
          <tbody>
            ...
            if (!(is_null($fantasquadraCalciatori))) {
              foreach ($fantasquadraCalciatori as $fantasquadraCalciatore) {

            ?>
                <tr>
                  <td><?php echo (($fantasquadraCalciatore["nome"])) ?></td>
                  <td><?php echo (($fantasquadraCalciatore["cognome"])) ?></td>
                  <td><?php echo (($fantasquadraCalciatore["ruolo"])) ?></td>
                  <td><?php echo ucwords(Fantacalcio::getNomeStemmaSquadra($fantasquadraCalciatore["id_squadra_serieA"])["nome"]) ?></td>
                  <td><?php echo (($fantasquadraCalciatore["valore"])) ?></td>
                  <td><?php echo (($fantasquadraCalciatore["costo_acquisto"])) ?></td>
                  <?php
                  echo '<td><a class="btn btn-danger" onclick=' . "'passDataSvincolaCalciatore(" . '"' . $fantasquadraCalciatore["nome"] . '","' . $fantasquadraCalciatore["cognome"] . '",' . $fantasquadraCalciatore["costo_acquisto"] . ',"' . Fantacalcio::getNomeStemmaFantasquadra($_GET["id_fantasquadra"])["nome"] . '",' . $fantasquadraCalciatore["id_fantasquadra"] . ',' . $fantasquadraCalciatore["id_calciatore"] . ")'" . ' data-toggle="modal" data-target="#modalDelete" role="button">Svincola</a></td>';
                  ?>
    ...
    <div class="modal fade" id="modalDelete" tabindex="-1" role="dialog" data-keyboard="false" data-backdrop="static" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <form method="POST" action="./CheckSvincolaCalciatori.php">
